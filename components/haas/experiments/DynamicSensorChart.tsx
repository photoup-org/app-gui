...
export default function DynamicSensorChart({ deviceId, telemetryData, deviceSchema, deviceName, experimentStatus }: DynamicSensorChartProps) {
                    ...
                    <Card key={schemaItem.key} className="border-slate-200 dark:border-slate-800 shadow-sm animate-in fade-in duration-500 overflow-hidden group">
                        <CardHeader className="pb-2 flex flex-row items-center justify-between">
                            ...
                            {(experimentStatus === "RUNNING" || experimentStatus === "PAUSED") && <div className="text-2xl font-bold">
                                {liveValues[schemaItem.key] !== undefined ? (
                                    liveValues[schemaItem.key].toFixed(2)
                                ) : (
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <LineChart data={metricData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                                            ...
                                            <Tooltip
                                                ...
                                                formatter={(value: any) => [`${value.toFixed(2)} ${schemaItem.unit}`, schemaItem.label]}
