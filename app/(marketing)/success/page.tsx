import { getOrderSuccessDetails } from "@/actions/success";
import ClearCartOnSuccess from "@/components/checkout/ClearCartOnSuccess";
import { Button } from "@/components/ui/button";
import { Download, AlertCircle, Package, CreditCard, MapPin, Check } from "lucide-react";
import Link from "next/link";

export default async function SuccessPage({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
    const result = await getOrderSuccessDetails(searchParams);
    console.log(result);
    if ("error" in result) {
        return (
    ...
    const { intentId, metadata, documentUrl, userEmail } = result.data;
    const cartItems = metadata.cartItems || [];
    const shippingAddress = metadata.shippingAddress || null;
    const billingAddress = metadata.billingAddress || null;

    return (
        <div className="min-h-screen py-16 px-4 sm:px-6 lg:px-8 flex flex-col items-centers">
            <ClearCartOnSuccess />
            ...
            <div className="max-w-4xl mx-auto w-full space-y-10 flex-1 items-center justify-center">
                ...
                {userEmail && (
                    <div className="bg-primary/5 border border-primary/20 rounded-xl p-5 text-center shadow-sm max-w-2xl mx-auto transition-colors hover:bg-primary/10">
                        <p className="text-base text-primary/90">
                            Um convite para aceder à plataforma foi enviado para <span className="font-bold text-primary">{userEmail}</span>.
                        </p>
                    </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between bg-card border rounded-xl p-6  gap-6">
                    <div className="space-y-1 text-center sm:text-left">
                        ...
                        <p className="text-xl font-mono font-semibold tracking-tight">{intentId}</p>
                    </div>
                    {documentUrl && (
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        {shippingAddress && (
                            <div className="bg-card border rounded-xl p-6 shadow-sm">
                                ...
                                <address className="not-italic text-sm text-muted-foreground space-y-1.5 leading-relaxed">
                                    <p className="font-medium text-foreground text-base">{shippingAddress.name}</p>
                        ...
                        {billingAddress && (
                            <div className="bg-card border rounded-xl p-6 shadow-sm">
                                ...
                                <address className="not-italic text-sm text-muted-foreground space-y-1.5 leading-relaxed">
                                    <p className="font-medium text-foreground text-base">{billingAddress.name}</p>
                                    {billingAddress.tax_id && <p>NIF: <span className="font-mono">{billingAddress.tax_id}</span></p>}
