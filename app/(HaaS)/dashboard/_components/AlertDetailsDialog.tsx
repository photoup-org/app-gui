...
const AlertDetailsDialog = ({ isOpen, onOpenChange, hours }: AlertDetailsDialogProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      const fetchAlerts = async () => {
        setIsFetching(true);
        try {
          const data = await getRecentAlertsAction(hours);
      ...
      <DialogContent className="sm:max-w-4xl max-w-[95vw] max-h-[85vh] flex flex-col p-0">
        <DialogHeader className="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <DialogTitle className="text-xl font-bold">
            Detalhes dos Alertas (Últimas {hours} {hours === 1 ? 'hora' : 'horas'})
          </DialogTitle>
        </DialogHeader>
